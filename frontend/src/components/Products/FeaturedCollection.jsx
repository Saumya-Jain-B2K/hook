// import React from 'react';
// import { Link } from 'react-router-dom';
// import featured from '../../assets/featured.webp';



// const FeaturedCollection = () => {
//   return (
//     <section className='py-16 px-4 lg:px-0'>
//         <div className='container mx-auto flex flex-col-reverse lg:flex-row items-center bg-green-50 rounded-3xl'>
//             {/* Left Content */}
//             <div className='lg:w-1/2 p-8 text-center lg:text-left'>
//             <h2 className='text-lg font-semibold text-gray-700 mb-2'>
//                 Comfort and Style
//             </h2>
//             <h2 className='text-4xl lg:text-5xl font-bold mb-6'>
//                 Apparel made for your everyday life
//             </h2>
//             <p className='text-lg text-gray-600 mb-6'>
//                 Discover high-quality, comfortable clothing that effortlessly blends
//                  fashion and functions. Designed to make you look and feel great everyday. 
//             </p>
//             <Link 
//             to="/collections/all" 
//             className="bg-[#13202d] px-6 py-3 rounded-lg text-white text-lg hover:bg-gray-800"
//             >
//                 Shop Now
//             </Link>
//             </div>

//             {/* Right Content */}
//             <div className='lg:w-1/2'>
//             <img src={featured} alt="Featured Collection" 
//             className='w-full h-full object-cover lg:rounded-tr-3xl lg:rounded-br-3xl'
//             />
//             </div>
//         </div>
//     </section>
//   )
// }

// export default FeaturedCollection

import React from 'react';
import { Link } from 'react-router-dom';
import featured from '../../assets/featured.webp';

const FeaturedCollection = () => {
  return (
    <section className="py-16 px-4 lg:px-0">
      {/* 
        Outer container:
        - Flex container with column-reverse on mobile so text appears before image for better UX
        - Row layout on large screens
        - Green background with rounded corners for styling
      */}
      <div className="container mx-auto flex flex-col-reverse lg:flex-row items-center bg-green-50 rounded-3xl overflow-hidden">
        
        {/* LEFT CONTENT: Text and Call-to-Action */}
        <div className="lg:w-1/2 p-6 sm:p-8 text-center lg:text-left">
          {/* Section Subtitle */}
          <h2 className="text-base sm:text-lg font-semibold text-gray-700 mb-2">
            Comfort and Style
          </h2>

          {/* Main Heading */}
          <h2 className="text-3xl sm:text-4xl lg:text-5xl font-bold mb-6">
            Apparel made for your everyday life
          </h2>

          {/* Description */}
          <p className="text-sm sm:text-base md:text-lg text-gray-600 mb-6 leading-relaxed">
            Discover high-quality, comfortable clothing that effortlessly blends
            fashion and function. Designed to make you look and feel great every day.
          </p>

          {/* CTA Button */}
          <Link
            to="/collections/all"
            className="bg-[#13202d] px-5 sm:px-6 py-2.5 sm:py-3 rounded-lg text-white text-sm sm:text-lg hover:bg-gray-800 transition-colors duration-200"
          >
            Shop Now
          </Link>
        </div>

        {/* RIGHT CONTENT: Featured Image */}
        <div className="lg:w-1/2 w-full">
          <img
            src={featured}
            alt="Featured Collection"
            className="w-full h-auto object-cover 
                       lg:rounded-tr-3xl lg:rounded-br-3xl"
          />
        </div>
      </div>
    </section>
  );
};

export default FeaturedCollection;
